        ...
        if (message != null) {
            when(Operation.fromInt(message.operation)) {
                ...
                Operation.CREATE_DAO -> {
                    ...
                    val blocks :List<TrustChainBlock>? = getCoinCommunity().fetchLatestJoinedSharedWalletBlocks()
                    val ids: List<String>? = blocks!!.map {it.blockId}
                    commandListener.send(Klaxon().toJsonString(Message(Operation.SEND_NEW_DAO_ID.op, Klaxon().toJsonString(
                        ParamsDaoIdsResponse(ids!!)
                    ))))
                }
                Operation.SEND_JOIN_PROPOSAL -> {
                    // val param: String = message.params;
                    if(message.params is String){
                        logger.error(message.params)
                        val daoIdObject: DAOId? = Klaxon().parse<DAOId>(message.params)
                        
                        val id: String = daoIdObject!!.daoId
                        logger.error(id)
                        
                        // send dao join proposal to the id
                        val blocks :List<TrustChainBlock>? = getCoinCommunity().discoverSharedWallets()
                        val wantedBlocks :List<TrustChainBlock>? = blocks!!.filter { x -> x.blockId == id }
                        logger.error(blocks!!.size.toString())
                        logger.error(blocks!![0].blockId)
                        logger.error(wantedBlocks!!.size.toString())
                        // logger.error(block??.blockId)
...
//        val walletDir = File("wallet-" + myPeer.publicKey)
        ...
        val walletDir = File("/home/kheoss/UniStuff/Blockchain/CSE4110_jre/coin/wallet-" + myPeer.publicKey)
        walletDir.mkdir()
